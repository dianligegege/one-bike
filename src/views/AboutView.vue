<template>
  <div class="container">
    <div class="input-container">
      <a @click="handleScan">点击</a>
      <div>
        {{ data.qrCode }}
      </div>
      <!-- <a-input-search ref="qrInput" v-model="data.qrCode" autofocus placeholder="请扫描二维码" search-button class="w-p-90"
        @change="searchInfo" @press-enter="searchInfo" @search="searchInfo">
        <template #prefix>
          <a @click="handleScan">点击</a>
        </template>
      </a-input-search> -->
    </div>
    <GetQrcode v-if="data.isQrCodeShown" @success="gotQrCode" @closeScan="closeScan" />
